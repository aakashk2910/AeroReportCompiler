            ...
            <div class="col-md-12">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">ADLI Table </h3>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <table class="table table-bordered">
                            <tr>
                                ...
                                <th>Topic</th>
                                <th>Topic No.</th>
                                <th style="width: 10px">A<small>(Approach)</small></th>
                                <th style="width: 10px">D<small>(Deployment)</small></th>
                                <th style="width: 10px">L<small>(Learning)</small></th>
                                <th style="width: 10px">I<small>(Integration)<small></th>
                            </tr>
                            <?php
                                $A=0;
                                $D=0;
                                ...
                                @foreach($data1 as $subCriteria)
                                    @if($subCriteria->criteriaId == $criteriaId)
                                        ...
                                        @foreach($flabel as $fl)
                                            <?php $text = DB::table('forminputs')->where('subCriteriaId', $subCriteria->id)->where('labelId', $fl->labelId)->where('userId', \Illuminate\Support\Facades\Auth::user()->id)->where('reportId', $reportId)->get(); ?>
                                            <?php $i = 'a'; ?>
                                            @foreach($text as $s)
                                                <?php ++$T; ?>
                                                    <tr>
                                                        ...
                                                        <td>{!! $fl->labelText !!}</td>
                                                        <td>{!! $i !!}</td>
                                                        <td>@if($s->A) <i class="fa fa-check"></i> <?php ++$A ?> @endif</td>
                                                        <td>@if($s->D) <i class="fa fa-check"></i> <?php ++$D ?> @endif</td>
                                                        <td>@if($s->L) <i class="fa fa-check"></i> <?php ++$L ?> @endif</td>
                                                        <td>@if($s->I) <i class="fa fa-check"></i> <?php ++$I ?> @endif</td>
                                                    </tr>
                                                <?php $i++; ?>
                                            @endforeach
                                        @endforeach
